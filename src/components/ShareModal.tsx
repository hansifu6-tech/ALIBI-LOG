...
export function ShareModal({ isOpen, onClose, records, tags, onStartGenerate }: ShareModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-opacity duration-200">
      <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl border border-gray-200 flex flex-col max-h-[90vh] overflow-hidden transition-all duration-200">
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-bold !text-gray-900">海报设置</h3>
          </div>
          <button 
            onClick={onClose} 
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-bold !text-gray-900">模式</span>
              </div>
              <div className="flex p-1 bg-gray-100 rounded-xl w-48 relative">
                <button
                  onClick={() => setShareType('calendar')}
                  className={`flex-1 relative z-10 py-1.5 text-sm font-bold transition-colors duration-200 ${
                    shareType === 'calendar' ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  📅 日历版
                </button>
                <button
                  ...
                  className={`flex-1 relative z-10 py-1.5 text-sm font-bold transition-colors duration-200 ${
                    shareType === 'achievement' ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  🏆 总结版
                </button>
                <div 
                  className={`absolute top-1 bottom-1 left-1 w-[calc(50%-4px)] bg-white rounded-lg shadow-sm transition-all duration-300 ease-out transform ${
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-bold !text-gray-900">颜色</span>
              </div>
              <div className="flex p-1 bg-gray-100 rounded-xl w-48 relative">
                <button
                  onClick={() => setPosterTheme('light')}
                  className={`flex-1 relative z-10 py-1.5 text-sm font-bold transition-colors duration-200 ${
                    posterTheme === 'light' ? 'text-amber-600' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  ☀️ 浅色
                </button>
                <button
                  ...
                  className={`flex-1 relative z-10 py-1.5 text-sm font-bold transition-colors duration-200 ${
                    posterTheme === 'dark' ? 'text-blue-500' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  🌙 深色
                </button>
          ...
          <section>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  <select
                    ...
                    className="appearance-none bg-transparent hover:bg-gray-100 px-1 py-0.5 rounded text-sm font-bold text-center cursor-pointer outline-none focus:ring-1 ring-blue-500/30 transition-all"
                  >
                      ...
                      return (
                        <button
                          ...
                          className={`
                            ...
                            ${start ? 'rounded-r-none ring-2 ring-blue-400 ring-offset-1' : ''}
                            ${end ? 'rounded-l-none ring-2 ring-blue-400 ring-offset-1' : ''}
            ...
            {startValue && (
              <div className="mt-3 flex justify-between text-[11px] font-bold !text-gray-500 px-2">
                <span className="!text-gray-700">开始: {startValue.toLocaleDateString()}</span>
                {endValue && <span className="!text-gray-700">结束: {endValue.toLocaleDateString()}</span>}
              </div>
            )}
          </section>
          ...
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => setSelectedHabitIds(habits.map(h => h.id))} className="text-[10px] font-bold px-2 py-1 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors !text-gray-800">全选</button>
                <button onClick={() => setSelectedHabitIds([])} className="text-[10px] font-bold px-2 py-1 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors !text-gray-800">全不选</button>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {habits.map(habit => (
                ...
                >
                  ...
                  <span className="text-xs font-bold truncate tracking-tight !text-gray-900">{habit.content}</span>
                </button>
              ))}
            </div>
          </section>
...
{/* 
          // Tag Selection (Temporarily hidden for Habit-Only focus)
          <section>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 !text-gray-900">
              ...
              <div className="flex gap-2 shrink-0">
                <button onClick={() => setSelectedTagIds(tags.map(t => t.id))} className="text-[10px] font-bold px-2 py-1 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors !text-gray-800">全选</button>
                <button onClick={() => setSelectedTagIds([])} className="text-[10px] font-bold px-2 py-1 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors !text-gray-800">全不选</button>
              </div>
            </div>
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl border-2 transition-all
                    ${selectedTagIds.includes(tag.id) 
                      ...
                      : 'border-gray-100 bg-white opacity-60'}`}
                >
                  <span className="text-xs font-bold !text-gray-900">#{tag.name}</span>
                </button>
              ))}
            </div>
          </section>
        ...
        <div className="p-6 border-t border-gray-100 flex gap-4 bg-gray-50/30">
          <button 
            onClick={onClose}
            disabled={isGenerating}
            className="flex-1 py-4 bg-gray-100 hover:bg-gray-200 !text-gray-800 font-bold rounded-2xl transition-all disabled:opacity-50"
          >
            取消
          </button>
          <button 
          ...
          >
            {isGenerating ? (
              <>
                ...
                <span className="!text-white">正在准备数据...</span>
              </>
            ) : (
              <span className="!text-white">开始生成</span>
            )}
          </button>
        </div>
